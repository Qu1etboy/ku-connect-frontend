...
import { data, defaultProfile, fieldOfStudy, formSchema, ProfileForm } from "./mock/data";
...
export default function OnBoardingPage() {
	...
	const validate = () => {
		const ids: any = current.group?.flatMap((group) => group.form?.map((field) => field.id));
		if (!ids) return true;
	...
	useEffect(() => {
		...
		if (faculty && data.steps?.[0]?.group?.[0]?.form) {
			const departmentField = data.steps[0].group[0].form.find((field) => field.id === "department");
			if (departmentField) {
		...
		<AuthProtected>
			<div className="max-h-screen min-h-screen flex flex-col px-10 py-10 bg-gradient-to-b from-white to-green-300 from-[50%]">
				<header>
					...
					<Progress value={(step / data.steps.length) * 100} max={data.steps.length} />
				</header>
				...
				<main className="flex-grow">
					<div className="flex flex-col items-center gap-3 h-[25vh] justify-evenly py-8">
						{isFirstStep ? (
							...
							<>
								<h1 className="text-2xl font-semibold items-center text-center">{current.title}</h1>
								<p className="text-sm text-muted-foreground text-center">{current.description}</p>
							</>
						)}
					</div>
					<div
